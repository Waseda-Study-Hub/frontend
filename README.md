...
FIRESTORE_DATABASE_ID=waseda-study-hub
...
Chat uses the named Firestore database `waseda-study-hub` directly from the
Firebase Web SDK. The FastAPI backend does not need messaging routes.

Before chat can be used, a Firebase project owner must deploy the included
participant-only security rules:

```bash
firebase deploy --only firestore --project waseda-study-hub
```

The included `firebase.json` targets the named `waseda-study-hub` database.
The rules require a verified Waseda email and allow only the two conversation
participants to read or send messages.
